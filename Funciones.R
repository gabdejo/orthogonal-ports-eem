...
out_of_sample_performance_implementable <- function(n, t, sigma, mu, gamma, implemented_type){
  set.seed(420)
  ...
  Y1 = ((n+1)/(t-n-1))*rf(n*t,n+1,t-n-1,t*sqr_shrp_H)
  Y2 = ((n-1)/(t-n-1))*rf(n*t,n-1,t-n-1,t*sqr_shrp_H)
  ...
  VH = k4 * sqr_shrp_H * mean(f1) + k5 * mean((f2^2)*Y2)
  VGH = k6 * mean(f2*Y2)
  Total = VG + VH + VGH
  results <- cbind(Total, VG, VH, VGH)
  colnames(results) <- c("Total", "Fund1", "Fund2", "Interaction")
  return(t(results))
}
...
out_of_sample_performance_theoretical <- function(n, t, sigma, mu, gamma, implemented_type){
  ...
  table1_result_S <- table1_portfolio_result(sigma, mu, gamma, "S")
...
performance_table <- function(df, type_table, n_obs, gamma){
  ...
  if (type_table == 'implementable') {
    results_implementable_KZ <- out_of_sample_performance_implementable(n, t, sigma, mu, gamma, "KZ")*100
    results_implementable_Q <- out_of_sample_performance_implementable(n, t, sigma, mu, gamma, "Q")*100
    results_implementable_M <- out_of_sample_performance_implementable(n, t, sigma, mu, gamma, "M")*100
    results_implementable_QS <- out_of_sample_performance_implementable(n, t, sigma, mu, gamma, "QS")*100
    results_implementable_QSa <- out_of_sample_performance_implementable(n, t, sigma, mu, gamma, "QSa")*100 
    ## joint results ---------------------------------------------------------------
  ...
  if (type_table == 'theoretical') {
    ...
    ## joint results ---------------------------------------------------------------
    results_theoretical <- cbind(results_theoretical_KZ,
                                      results_theoretical_Y,
                                      results_theoretical_Q, 
                                      results_theoretical_M)
    colnames(results_theoretical) <- c("KZ","Y","Q","M")
    rownames(results_theoretical) <- c("Total")
    return(results_theoretical)
  }
}

copulas_test <- function(df,n_obs ,n_sim, n_rep){
  df_copulas <- df %>% select(-date) 
  df_copulas_pseudo <- pobs(df_copulas)
  ncol <- ncol(df_copulas_pseudo)
  fitcopula_gabo <- fitCopula(tCopula(dim=ncol,dispstr='un'), df_copulas_pseudo,
                              method = 'ml')
  ...
  list_result <- lapply(list_sim, performance_table, 'implementable', n_obs, gamma = 1)
  ...
  lista <- list(df_simulation,df_simulation_colmeans,ttest,wilcoxtest)
  return(lista)
}

sensitivity <- function(df, vector_sensitivity){
  df_sensitivity <- df %>% select(-date) 
  list_result <- lapply(list_sim, performance_table, df = df_sensitivity, 
